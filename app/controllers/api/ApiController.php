...

{
...

    {
        ...
        $secretKey = $_ENV['PDF_SIGNING_SECRET_KEY'] ?? null;
        ...
        if (empty($secretKey)) {
            error_log("PDF_SIGNING_SECRET_KEY is not set in environment variables");
            http_response_code(500);
...

    {
        ...
        if (empty($secretKey)) {
            error_log("PDF_SIGNING_SECRET_KEY is not set in environment variables");
            throw new \Exception("PDF signing secret key is not configured. Please set PDF_SIGNING_SECRET_KEY in your .env file.");
        }
    ...
    public function getTicketDetails($ticketNumber)
    {
        ...
        try {
            ...
            if (!$ticket) {
                ...
                if ($trengoInfo !== null) {
                    if (is_array($trengoInfo) && !isset($trengoInfo['error'])) {
                        // Valid helper data - add it
                        $response['helper'] = $trengoInfo;
                    } elseif (is_array($trengoInfo) && isset($trengoInfo['error'])) {
                        // There was an error, but still include it for debugging
                        $response['helper'] = $trengoInfo;
                        error_log("Trengo API error for ticket {$ticketNumber}: " . $trengoInfo['error']);
                    }
                } else {
                    // Token not configured or API unavailable
                    error_log("Trengo API not available for ticket {$ticketNumber} - token may not be configured or API call failed");
                }
