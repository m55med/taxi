...
<div class="mb-6">
    ...
    <label for="folder_id" class="block text-gray-700 text-sm font-bold mb-2">Folder (Optional):</label>

    <select id="folder_id" name="folder_id" class="shadow border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500">

        <option value="">-- No Folder (General) --</option>

        <?php foreach ($folders ?? [] as $folder) : ?>

            <option value="<?= $folder['id'] ?>" <?= (isset($article['folder_id']) && $article['folder_id'] == $folder['id']) ? 'selected' : '' ?>

                    style="color: <?= htmlspecialchars($folder['color']) ?>;">

                <i class="<?= htmlspecialchars($folder['icon']) ?> mr-2"></i>
    ...
    <p class="text-xs text-gray-500 mt-1">Choose a folder to organize this article. Leave empty for general articles.</p>
