                ...
                <table class="min-w-full leading-normal">
                    ...
                    <tbody>
                        <?php if (empty($data['notifications'])): ?>
                            <tr>
                                <td colspan="5"
                                    class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center text-gray-500">No
                                    notifications have been sent yet.</td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap font-semibold">
                                            <?= htmlspecialchars($notification['title']) ?></p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <div class="max-w-xs truncate"
                                            title="<?= htmlspecialchars(strip_tags($notification['message'])) ?>">
                                            <?= htmlspecialchars(strip_tags($notification['message'])) ?>
                                        </div>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap">
                                            <?= date('Y-m-d H:i', strtotime($notification['created_at'])) ?></p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap"><?= $notification['total_read'] ?> /
