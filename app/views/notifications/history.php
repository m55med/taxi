    ...
    <div class="space-y-6">
        ...
        <?php else: ?>
            <?php foreach($data['notifications'] as $notification): ?>
                <div id="notification-<?= $notification['id'] ?>" class="bg-white shadow-md rounded-lg p-6 border-l-4 <?= $notification['is_read'] ? 'border-gray-300' : 'border-blue-500' ?> scroll-mt-20">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                        <h2 class="text-xl font-bold text-gray-800 mb-2 sm:mb-0"><?= htmlspecialchars($notification['title']) ?></h2>
                        <span class="text-xs text-gray-500"><?= date('F j, Y, g:i a', strtotime($notification['created_at'])) ?></span>
                    </div>
                    ...
                    <?php if (!$notification['is_read']): ?>
                        <div class="mt-4 text-right">
                            <span class="inline-block bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full">NEW</span>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
