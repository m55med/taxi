...
<body class="bg-gray-100" x-data="platformsPage(<?= htmlspecialchars(json_encode($flashMessage), ENT_QUOTES) ?>)" x-init="init()" x-cloak>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1">
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4" x-text="form.id ? 'Edit Platform' : 'Add New Platform'">Add New Platform</h2>
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="flex items-center mt-4 space-x-2">
                            <button type="submit" class="w-full bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 flex items-center justify-center">
                                ...
                                <span class="ml-2" x-text="form.id ? 'Save Changes' : 'Add Platform'"></span>
                            </button>
                                        ...
                                        <?php foreach ($data['platforms'] as $index => $platform): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                                    <button @click="editPlatform(<?= htmlspecialchars(json_encode($platform)) ?>)" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <button @click="confirmDelete(<?= $platform['id'] ?>, '<?= htmlspecialchars(addslashes($platform['name'])) ?>')" class="text-red-600 hover:text-red-900" title="Delete">
...
<script>
function platformsPage(flashMessage) {
    return {
