...
if (isset($_SESSION['ticket_category_message'])) {
    $flashMessage = [
        'message' => $_SESSION['ticket_category_message'],
...
<div x-data="ticketCategoriesPage(<?= htmlspecialchars(json_encode($flashMessage), ENT_QUOTES) ?>)" x-init="init()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-1">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <form action="<?= BASE_PATH ?>/admin/ticket_categories/store" method="POST">
                        <div>
                                        ...
                                        <?php foreach ($data['ticket_categories'] as $index => $category): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                                    <button @click="confirmDelete(<?= $category['id'] ?>, '<?= htmlspecialchars(addslashes($category['name'])) ?>')" class="text-red-600 hover:text-red-900" title="Delete">
...
<script>
function ticketCategoriesPage(flashMessage) {
    return {
        ...
        confirmDelete(id, name) {
            this.deleteModal.message = `Are you sure you want to delete the category "<strong>${name}</strong>"? This may not be possible if it has subcategories.`;
            this.deleteModal.actionUrl = `<?= BASE_PATH ?>/admin/ticket_categories/delete/${id}`;
