        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1">
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4" x-text="form.id ? 'Edit Document Type' : 'Add New Document Type'">Add New Document Type</h2>
                    <form @submit.prevent="submitForm">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Document Type Name</label>
                            <input type="text" name="name" id="name" x-model="form.name" required class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="flex items-center mt-4 space-x-2">
                            ...
                            <button type="button" x-show="form.id" @click="resetForm()" class="w-auto bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 font-medium">
                                Cancel
                            </button>
                        </div>
                    </form>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($data['document_types'] as $index => $docType): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= htmlspecialchars($docType['name']) ?></td>
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                                    <button @click="editType(<?= htmlspecialchars(json_encode($docType)) ?>)" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <button @click="confirmDelete(<?= $docType['id'] ?>, '<?= htmlspecialchars(addslashes($docType['name'])) ?>')" class="text-red-600 hover:text-red-900" title="Delete">
    ...
    return {
        ...
        submitForm() {
            ...
            formRef.action = this.form.id 
                ? `<?= BASE_PATH ?>/admin/document_types/update` 
                : `<?= BASE_PATH ?>/admin/document_types/store`;
            formRef.submit();
        },
        ...
        confirmDelete(id, name) {
            ...
            this.deleteModal.actionUrl = `<?= BASE_PATH ?>/admin/document_types/delete/${id}`;
