...
$flashMessage = null;
if (isset($_SESSION['ticket_subcategory_message'])) {
    $flashMessage = [
        'message' => $_SESSION['ticket_subcategory_message'],
        'type' => $_SESSION['ticket_subcategory_message_type'] ?? 'success'
    ];
    unset($_SESSION['ticket_subcategory_message'], $_SESSION['ticket_subcategory_message_type']);
}
?>
<?php include_once __DIR__ . '/../../includes/header.php'; ?>

<div x-data="ticketSubcategoriesPage(<?= htmlspecialchars(json_encode($flashMessage), ENT_QUOTES) ?>)" x-init="init()">
    ...
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Manage Subcategories</h1>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-1">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">Add New Subcategory</h2>
                    <form action="<?= BASE_PATH ?>/admin/ticket_subcategories/store" method="POST">
                        <div class="mb-4">
            ...
            <div class="md:col-span-2">
                <div class="bg-white rounded-lg shadow-sm">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h2 class="text-xl font-semibold text-gray-800">Existing Subcategories</h2>
                    </div>
                    <div class="p-0">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subcategory Name</th>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Category</th>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <?php if (empty($data['ticket_subcategories'])): ?>
                                        <tr>
                                    ...
                                    <?php else: ?>
                                        ...
                                        ?>
                                        <?php foreach ($data['ticket_subcategories'] as $index => $subcategory): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                                    <button @click="confirmDelete(<?= $subcategory['id'] ?>, '<?= htmlspecialchars(addslashes($subcategory['name'])) ?>')" class="text-red-600 hover:text-red-900" title="Delete">
...
<script>
function ticketSubcategoriesPage(flashMessage) {
    return {
        ...
        confirmDelete(id, name) {
            this.deleteModal.message = `Are you sure you want to delete the subcategory "<strong>${name}</strong>"? This may not be possible if it has codes associated with it.`;
            this.deleteModal.actionUrl = `<?= BASE_PATH ?>/admin/ticket_subcategories/delete/${id}`;
