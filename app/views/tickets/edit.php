        ...
        <form action="<?= BASE_URL ?>/tickets/update/<?= $data['ticket']['id'] ?>" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="assigned_team_leader_id" id="assigned_team_leader_id" required class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm">
                        <?php foreach ($data['team_leaders'] as $leader): ?>
                            <option value="<?= $leader['id'] ?>" <?= ($leader['id'] == $data['ticket']['assigned_team_leader_id']) ? 'selected' : '' ?>>
