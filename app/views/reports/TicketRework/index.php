            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">عنوان التذكرة</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">اسم الموظف</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">مرات إعادة الفتح</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">تاريخ الإغلاق</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach($data['reworks'] as $rework): ?>
                    <tr>
                        <td class="px-6 py-4"><?= htmlspecialchars($rework['id']) ?></td>
                        <td class="px-6 py-4"><?= htmlspecialchars($rework['subject']) ?></td>
                        <td class="px-6 py-4"><?= htmlspecialchars($rework['agent_name']) ?></td>
                        <td class="px-6 py-4 font-bold text-red-600"><?= htmlspecialchars($rework['reopened_count']) ?></td>
                        <td class="px-6 py-4"><?= date('Y-m-d H:i', strtotime($rework['closed_at'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
