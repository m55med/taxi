function createTicketForm(platforms, marketers) {
    return {
        ...
        async fetchTicketData() {
            if (!this.formData.ticket_number) return;

            // First check if it exists in our DB
            await this.checkTicketExists();

            // Then, fetch from Trengo
            try {
                toastr.info('Fetching ticket data from Trengo...');
                const response = await fetch(`${URLROOT}/create_ticket/fetch_trengo_ticket/${this.formData.ticket_number}`);
                
                if (!response.ok) {
                    const errorResult = await response.json().catch(() => ({ message: 'Invalid JSON response from server.' }));
                ...
                if (data.success && data.data && typeof data.data === 'object') {
                    ...
                    this.formData.phone = ticketData.contact?.phone || '';

                    let notes = '';
                    if (ticketData.audits && Array.isArray(ticketData.audits) && ticketData.audits.length > 0) {
                        // The user wants a summary of the latest events from the 'audits' array.
                        notes = ticketData.audits
                            .slice(0, 5) // Take the 5 most recent audit logs
                            .map(audit => `[${audit.created_at}] ${audit.message}`)
                            .join('\n');
                    }
                    this.formData.notes = notes;

                    toastr.success('Ticket data auto-filled from Trengo.');
                } else if (!data.success) {
                    toastr.warning(data.message || 'Could not fetch ticket data from Trengo.');
                } else {
                    toastr.warning('Ticket found on Trengo, but no data was returned or data is invalid.');
                }

            } catch (error) {
                toastr.error(`Error fetching from Trengo: ${error.message}`);
                console.error('Error fetching ticket data from Trengo:', error);
            }
        },
